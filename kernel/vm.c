...
walkaddr(pagetable_t pagetable, uint64 va)
{
  ...
  if(pte == 0)//虚拟地址对应的PTE为0时,是没有PTE的
    page_grant = 0;
  else
  {
    if((*pte & PTE_V) == 0)
...
void vwalk(pagetable_t pageable, int depth)
{
    for (int i = 0; i < 512; ++i)
    {
        pte_t pte = pageable[i];
        if (pte & PTE_V)
        {
        ...
        {
            uint64 child = PTE2PA(pte);
            vwalk((pagetable_t)child, depth+1);
        }
    }
}
